...
export default function SettingsButton(selectedAudioDevice: any, selectedVideoDevice: any) {
  ...
  const [selectedSpeaker, setSelectedSpeaker] = useState(selectedSpeakerOutput ? selectedSpeakerOutput : '');
  ...
  const { devices: audioinputs } = useMediaDevices('audioinput');

  const { devices: videoinputs } = useMediaDevices('videoinput');

  const { devices: audiooutput } = useMediaDevices('audiooutput');
      ...
      >
        ...
        <DialogContent>
          <FormControl>
            ...
            >
              {audioinputs
                .filter((x: { deviceId: string }) => x.deviceId !== 'default')
                .map((input: any) => (
